...
import { AstBuilder, dialects, Parser } from '@cucumber/gherkin';
import callsites from 'callsites';
import { v4 as uuidv4 } from 'uuid';

import type { Options } from './configuration';
import { getCucumberConfiguration } from './configuration';
import type {
  ParsedFeature,
  ParsedScenario,
  ...
  return scenarioSteps.map((scenarioStep) => {
    const stepText = Object.keys(exampleTableRow).reduce(
      (processedStepText, nextTableColumn) => {
        return processedStepText.replace(
    ...
    let stepArgument: string | Record<string, unknown> = '';

    if (scenarioStep.stepArgument) {
      if (Array.isArray(scenarioStep.stepArgument)) {
        stepArgument = (scenarioStep.stepArgument as any).map(
          (stepArgumentRow: any) => {
...
const parseScenarioOutlineExample = (
  ...
  return {
    ...
    tags: Array.from(
      new Set<string>([...outlineScenario.tags, ...exampleSetTags])
    ),
  } as ParsedScenario;
};
...
const parseScenarioOutlineExampleSets = (
  ...
  return exampleSets.reduce(
    (scenarios: ParsedScenario[], nextExampleSet: ParsedScenario[][]) => {
...
const parseFeature = (
  ...
  let ast: any;
  options = getCucumberConfiguration(options);

  try {
    const builder = new AstBuilder(uuidv4 as any);
    ast = new Parser(builder).parse(featureText);
  } catch (err) {
    throw new Error(`Error parsing feature Gherkin: ${(err as any).message}`);
  }
...
export const loadFeature = (featureFilePath: string, options?: Options) => {
  ...
  const absoluteFeatureFilePath = resolve(
    options && options.loadRelativePath ? dirOfCaller : '',
    featureFilePath
  );
